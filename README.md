...
team rubric exempts these from the 100% goal. They are documented in this
section.
...
| `advanceSetupTurn` | Implicit loop fallthrough at the closing brace | `isSetupComplete()` was checked false immediately before the loop, so at least one player still has armies to place. The loop iterates a full rotation and is guaranteed to return from inside before the natural exit condition becomes true. |
...
| `tradeCards`, `tradeSetCount <= TRADE_BONUS_TABLE.length` | Conditional boundary (`<=` → `<`) | The bonus table's last entry (`TRADE_BONUS_TABLE[5] = 15`) equals the formula evaluated at the boundary (`15 + TRADE_BONUS_INCREMENT * (6 - 6) = 15`). At every `tradeSetCount` value, both the original and the mutated code produce the same bonus, so no test can detect the change. |
| `advanceSetupTurn`, `for (int i = 1; i <= players.size(); i++)` | Conditional boundary (`<=` → `<`) | The loop always returns from inside on or before its `players.size()`-th iteration (see the JaCoCo entry above). The boundary mutation only changes behavior on an iteration that never executes. |
